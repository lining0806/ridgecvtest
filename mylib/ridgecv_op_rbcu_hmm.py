...
class predict():
    ...
    def dataProcess(self, load_data=True):
        if load_data is True:
            ...
            # self.savePairData()
            self.data = self.loadPairData() # Read in data
            # print self.data
            ## ---------------------------------------------------------------------------------------
            # day_night_list = []
            ...
            # with open('./data_feature', 'wb') as f    p:
    ...
    def loadPairData(self, fname='./data/rb-cu/pairs.csv'):
        assert self.skip_rows >= self.nrows
        def count_lines(filename):
            count = 0
            buffer_size = 1024*1024
        ...
        # lines = np.sum(1 for _ in csv.reader(open(fname)))
        # print lines
        if lines < self.skip_rows:
            self.skip_rows = lines
        if lines < self.nrows:
    ...
    def savePairData(self, fname='./data/rb-cu/pairs.csv', fname1='./data/rb-cu/cu09_16.csv', fname2='./data/rb-cu/rb09_16.csv', ratio=-10):
        data1 = self.loadSingleData(fname1)
        data2 = self.loadSingleData(fname2)
        # data = data1+data2*ratio
        data = data1.copy()
        data['Open'] += data2['Open']*ratio
        data['High'] += data2['High']*ratio
        data['Low'] += data2['Low']*ratio
        data['Close'] += data2['Close']*ratio
        # print data.shape
        data.dropna(inplace=True)
        data.to_csv(fname)

    def loadSingleData(self, fname):
        assert self.skip_rows >= self.nrows
        def count_lines(filename):
            count = 0
            buffer_size = 1024*1024
    ...
    def targetDefine_ratio(self, threshold=0.0):
        ratio = 0.2
        self.data['Diff'] = self.data.Close.diff(self.timeshift).shift(-self.timeshift).fillna(0)
        sorted_Diff = np.sort(self.data['Diff'].abs().values)
        threshold_index = int(len(sorted_Diff)*ratio)
        ...
        sorted_Diff_ = np.sort(self.data['Diff_'].abs().values)
        threshold_index = int(len(sorted_Diff_)*ratio)
        threshold = sorted_Diff_[threshold_index]
        ...
        if mode == 'forward':
            ...
            for i_cross_predict in np.arange(self.cross_predict_num):
                ...
                #     y_pred = 0
                        ...
                        n_components = 3
                        # model = GMMHMM(n_components, covariance_type='tied', algorithm='viterbi', n_iter=100)
                        model = GaussianHMM(n_components, covariance_type='tied', algorithm='viterbi', n_iter=100)
                        model.fit(self.X_train)
                        Regression = model
                        train_states = Regression.predict(self.X_train)
                y_state_pred = Regression.predict(self.X_test.reshape(1, -1))[-1] # Only one predict in the result
                y_train_select = self.y_train[train_states == y_state_pred]
                y_train_dict = {}
                for y_train in y_train_select:
                    y_train_dict[y_train] = y_train_dict.get(y_train, 0)+1
                sorted_items = sorted(y_train_dict.items(), key=lambda xx:xx[1], reverse=True)
                y_pred = sorted_items[0][0]
...
if __name__ == '__main__':
    ...
    size1 = np.sum(temp.data.groupby(temp.data.Date).size()['2015-11-01':])
